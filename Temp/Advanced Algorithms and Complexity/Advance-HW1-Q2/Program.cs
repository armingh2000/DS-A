...
namespace Advance_HW1_Q2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Solve(n, m, matrix).ToList().ForEach(x => Console.Write(x + " "));
        }
        ...
        {
            Graph g = new Graph(flightCount + crewCount + 2);

            for (long i = 1; i <= flightCount; i++)
            {
                g.AddEdge(0, i, 1);
            }

            for (long j = 1; j <= crewCount; j++)
            {
                g.AddEdge(flightCount + j, flightCount + crewCount + 1, 1);
            }
            ...
            for (long i = 0; i < flightCount; i++)
            {
                ...
                for (long j = 0; j < crewCount; j++)
                {
                    if (info[i][j] == 1)
                    {
                        g.AddEdge(i + 1, flightCount + 1 + j, 1);
                    }
            ...
            return g.BipartiteMatching(0, flightCount + crewCount + 1, flightCount);
        }
